...
bool comparator (const Homework &h1, const Homework &h2) {
...
private:
    ...
    int get_result() {
        // Aflati punctajul maxim pe care il puteti obtine planificand
        // optim temele.

        sort(hws.begin(), hws.end(), comparator);
        int result = 0, weeks = 0;

        for (auto hw : hws) {
            if (weeks <= hw.deadline) {
                weeks++;
                result += hw.score;
            }
        }
